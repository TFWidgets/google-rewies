 ...
 * - GITHUB_REPO - Имя репозитория (например: TFWidgets/google-rewies)
 */
    ...
    try {
        ...
        if (!env.GITHUB_TOKEN || !env.GITHUB_REPO) {
            ...
            if (env.GOOGLE_REVIEWS_CONFIGS) {
                ...
                await env.GOOGLE_REVIEWS_CONFIGS.put(sanitizedId, configJson);
                
                return new Response(JSON.stringify({ 
                    success: true,
                    configId: sanitizedId,
                    message: 'Configuration saved to KV storage (GitHub not configured)',
                    embedCode: `<script src="https://google-reviews.pages.dev/embed.js" data-id="${sanitizedId}"></script>`
                }), {
                    status: 200,
                    headers: { 
